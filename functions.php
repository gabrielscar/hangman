...
function getRating() {
  ...
  $rating = $correctGuesses / $incorrectGuesses;
  $grade = [
    [0.8, "A", "success"],
    [0.6, "B", "info"],
    [0.4, "C", "warning"],
...
function setUpGame() {
  ...
  if(!isset($_SESSION['word'])) {
    // grabing words from txt file, functia file() va genera un array
    // $words = file("words.txt");
    // Random word from the array, rtrim scoate caractere suplimentare din drapta cuvantului, lista adauga 2 caractere suplimentare
    $word = rtrim(strtoupper($words[array_rand($words)]));
    // Storing the word in session
    $_SESSION['word'] = $word;
    // array o guessing letters
    $_SESSION['guesses'] = [];
    $_SESSION['lives'] = 6;
    // storing  won games to the session
    if(!isset($_SESSION['gamesWon'])) {
      $_SESSION['gamesWon'] = 0;
    }
    // storing  lost games to the session
...
function dealWithGuess() {
  // Counting down lives on mistakes
  if(isset($_POST['guess'])) {
...
function currentStateOfPlay () {
  ...
  // finding the letters in the array
